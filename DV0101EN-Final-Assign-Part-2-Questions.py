...
def update_input_container(selected_statistics):
    if selected_statistics =='Yearly Statisics': 
        return False
    else: 
        return True
...
yas= data.groupby('Year')['Automobile_Sales'].mean().reset_index()
    ...
    x='Year',
    y='Automobile_Sales',
    title='AVG Yearly Automobile Sales'))
...
exp_data = yearly_data.groupby(['Vehicle_Type'])['Advertising_Expenditure'].sum().reset_index()
    ...
    exp_data, 
    values='Advertising_Expenditure',
    names='Verhicle_Types',
    title='Total Advertisment Expenditure for Each Vehicle'))

#TASK 2.6: Returning the graphs for displaying Yearly data
return [
    html.Div(className='chart-item', children=[html.Div(children=_Ychart1),html.Div(children=Y_chart2)],style={'display':'flex'}),
